                    ...
                    <>
                            ...
                            {contentObj.listWithNumbers.map(listObj =>
                              listObj.listWithAlpahbets ? (
                                ...
                                <ol type="a">
                                  ...
                                  {listObj.listWithAlpahbets.map(listObj1 => (
                                    <li>
                                      {listObj1.includes('$$')
                                        ? boldText(listObj1, classes)
                                        : listObj1}
                                    </li>
                                  ))}
                                </ol>
                      ...
                      {contentObj.paragraph && (
                        <div className={classes.text}>
                          {contentObj.paragraph
                            .split('$$')
                            .map(splitedParagraph => {
                              // Checking for regex ()[] pattern
                              if (
                                splitedParagraph != null &&
                                (/\[(.+)\]\((.+)\)/g.test(splitedParagraph) ||
                                  /\((.+)\)\[(.+)\]/g.test(splitedParagraph))
                              ) {
                                const title = splitedParagraph
                                  .match(/\[(.*)\]/)
                                  .pop();
                                const linkAttrs = splitedParagraph
                                  .match(/\((.*)\)/)
                                  .pop()
                                  .split(' ');
                                const target = linkAttrs.find(link =>
                                  link.includes('target:')
                                );
                                const url = linkAttrs.find(link =>
                                  link.includes('url:')
                                );
                                const type = linkAttrs.find(link =>
                                  link.includes('type:')
                                ); // 0 : no img
                              ...
                              if (
                                ...
                                const downloadAttrs = splitedParagraph
                                  .match(/{(.*)}/)
                                  .pop()
                                  .split(',');
                                const downloadLink = downloadAttrs.find(link =>
                                  link.includes('link:')
                                );
                                const downloadTitle = downloadAttrs.find(link =>
                                  link.includes('title:')
                                );
                                return (
                                  <Link
                                    ...
                                    href={
                                      downloadLink
                                        ? downloadLink.replace('link:', '')
                                        : ''
                                    }
                                  >
                                    {downloadTitle
                                      ? downloadTitle.replace('title:', '')
                                      : ''}
                                  </Link>
                                );
                              }
                              ...
                              <tr className={classes.tableBodyRow}>
                                ...
                                {contentObj.table[0].head.map(rowObj => {
                                  ...
                                  if (rowObj != null && /{(.*)}/.test(rowObj)) {
                                    const thAttrs = rowObj
                                      .match(/{(.*)}/)
                                      .pop()
                                      .split('$$');
                                    const inlineStyleStr = thAttrs
                                      .find(thAttr => thAttr.includes('style:'))
                                      .replace('style:', '')
                                      .replace(/'/g, '');
                                    ...
                                    const text = thAttrs.find(thAttr =>
                                      thAttr.includes('text:')
                                    );
                                    outputHTML = (
                                      <th
                                        className={classes.headerCell}
                                  ...
                                  <tr className={classes.tableBodyRow}>
                                    ...
                                    {rowObj.row.map(rowValue => {
                                      ...
                                      if (
                                        ...
                                        const thAttrs = rowValue
                                          .match(/{(.*)}/)
                                          .pop()
                                          .split('$$');
                                        const inlineStyleStr = thAttrs
                                          .find(thAttr =>
                                            thAttr.includes('style:')
                                          )
                                          .replace('style:', '')
                                          .replace(/'/g, '');
                                        const inlineStyleMap = {};
                                      ...
                                      if (
                                        ...
                                        const title = rowValue
                                          .match(/\[(.*)\]/)
                                          .pop();
                                        const linkAttrs = rowValue
                                          .match(/\((.*)\)/)
                                          .pop()
                                          .split(' ');
                                        const target = linkAttrs.find(link =>
...
const styles = () => ({
  container: {
    ...
    color: '#000000',
    fontFamily: props =>
      props.data.fontFamily ? props.data.fontFamily : 'Nunito',
    fontSize: '15px',
    lineHeight: '22px',
    maxWidth: '1440px',
  },
  ...
  text: {
    ...
    fontSize: '16px',
    lineHeight: props =>
      props.data.lineHeight ? props.data.lineHeight : '30px',
  },
  title: {
    color: '#0B3556',
    fontWeight: 'bold',
  ...
  contentSection: {
    padding: props =>
      props.data.imageLocation === 'right'
        ? '8px 25px 8px 0px !important'
        : '8px 0px 8px 25px !important',
    float: 'left',
  },
  imageSection: {
    float: 'left',
