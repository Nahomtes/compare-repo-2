...
const columns = [
  ...
  },
  {
    ...
    renderCell: ({ geneEnsemblId, EFO }) => (
      <Link to={`/evidence/${geneEnsemblId}/${EFO}`} external>
        Evidence Page
      </Link>
    ),
  },
...
function CHoPPage() {
  ...
  const [firstLoad, setFirstLoad] = useState(
    !isEmpty(geneSymbol) || !isEmpty(disease)
  );

  const [displayTable, setDisplayTable] = useState(
    false || geneSymbol.length !== 0 || disease.length !== 0
  );
  ...
  const handleOnClick = e => {
    if (inputFieldAreBothEmpty === false) {
      getData({
        variables: {
          disease: inputSanitize(diseaseInputValue),
  ...
  const reformatResult =
    result?.length !== 0 ? getRows(result?.pedCanNav.rows || []) : [];

  const resultInfoObj = () => {
    const searchOnlyForTarget =
      isEmpty(diseaseForInfo) && !isEmpty(targetForInfo);
    const searchOnlyForDisease =
      isEmpty(targetForInfo) && !isEmpty(diseaseForInfo);
        ...
        res = (
          <Typography component="p">
            ...
            {searchFor.target ? (
              <span>
                {' '}
                Diseases with <strong>{targetForInfo}</strong>{' '}
              </span>
            ) : (
              ''
            )}
            {searchFor.disease ? (
              <span>
                {' '}
                Targets with <strong>{diseaseForInfo}</strong>{' '}
              </span>
            ) : (
              ''
            )}
            {searchFor.both ? (
              <span>
                {' '}
                result of <strong>{targetForInfo}</strong> in{' '}
                <strong>{diseaseForInfo}</strong> with{' '}
              </span>
            ) : (
              ''
            )}{' '}
            pediatric cancer evidence data. Note that the existence of data does
            not necessarily indicate significance.
          </Typography>
        );
      } else {
  ...
  return (
    <div className={classes.page}>
      ...
      <NCIHeader />
      {/*       Body      */}
      <Grid container justify={'center'} className={classes.flexContainer}>
        <Grid item xs={12}>
          ...
          >
              ...
              >
                ...
                >
                  <Grid item xs>
                    ...
                    >
                      Search for a <b>Target</b>, <b>Disease</b>, or <b>both</b>{' '}
                      to navigate our dataset containing{' '}
                      <b>{NUMBER_OF_TARGET}</b> Targets and{' '}
                      <b>{NUMBER_OF_DISEASE}</b> Diseases across{' '}
                      <b>{NUMBER_OF_EVIDENCE}</b> Evidence Pages.
                    </Typography>
                  </Grid>
                </Grid>
              </Grid>
            ...
            <Grid container justifyContent="center" item xs={12}>
              ...
              >
                <Grid item className={classes.entityNameItem}>
                  {' '}
                  Gene Symbol:{' '}
                </Grid>
                <Grid item xs className={classes.geneSymbolSelectItem}>
                  <EntitySelect
                    ...
                    setInputValue={setTargetInputValue}
                    entity="target"
                    defaultOptions={defaultTargetOptions}
                  />
                </Grid>
              </Grid>
              {/*   Disease   */}
              ...
              >
                ...
                <Grid item xs className={classes.diseaseSelectItem}>
                  <EntitySelect
                    ...
                    setInputValue={setDiseaseInputValue}
                  />
                </Grid>
              </Grid>
              <Grid
              ...
              >
                <Grid item>
                  <Button
                    ...
                    disabled={inputFieldAreBothEmpty}
                    variant="contained"
                    size="large"
                  >
                    {' '}
