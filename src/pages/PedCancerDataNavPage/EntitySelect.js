...
const getDiseaseOptions = () => {
  ...
  DiseaseOptions.forEach(e => {
    diseaseList.push({ value: e.Disease, label: e.Disease });
  });
  return diseaseList;
};
    ...
    return (
      <components.NoOptionsMessage {...props}>
        <span className="custom-css-class">
          {' '}
          {inputvalue.length > 0 ? 'No matches were found' : placeHolder}
        </span>
      </components.NoOptionsMessage>
    );
  };
  const ClearIndicator = props => {
    return props?.selectProps?.value?.value?.length === 0 ? null : (
